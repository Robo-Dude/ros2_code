...
class VideoStitcher:
    def __init__(self):
        ...
        self.left_video = cv2.VideoCapture(12)
        self.right_video = cv2.VideoCapture(17)

        # Initialize the saved homography matrix
        self.saved_homo_matrix = None

    def stitch(self, images, ratio=0.75, reproj_thresh=90.0):
        # Unpack the images
    ...
    def run(self):
        ...
        ok, left = self.left_video.read()
        _, right = self.right_video.read()

        # Stitch the frames together to form the panorama
        stitched_frame = self.stitch([left, right])

        # stitched_frame = cv2.resize(stitched_frame, (2460, 720))
