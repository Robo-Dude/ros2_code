...
class autocontrollerPublisher(Node):

    def __init__(self):
        super().__init__('auto_controller_publisher')
        self.publisher_pwm_one = self.create_publisher(Int16, '/pwm1', 10)
        self.publisher_pwm_two = self.create_publisher(Int16, '/pwm2', 10)

        self.subscription1 = self.create_subscription(State, '/auto_state', self.autolistener_callback, 100)
        self.subscription1  # prevent unused variable warning

        self.subscription2 = self.create_subscription(State, '/state', self.listener_callback, 100)
...
def main(args=None):
    ...
    auto_controller_publisher = autocontrollerPublisher()

    rclpy.spin(auto_controller_publisher)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    auto_controller_publisher.destroy_node()
    rclpy.shutdown()
